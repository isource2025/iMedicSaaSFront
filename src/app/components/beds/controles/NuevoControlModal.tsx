...
interface Props {
	...
	onClose: () => void;
	/** Si se pasa, el modal edita ese control en lugar de crear uno nuevo. */
	controlToEdit?: ControlFrecuente | null;
}

export default function NuevoControlModal({
	defaultNumeroVisita,
	refetch,
	onClose,
	controlToEdit = null,
	...
	const isEdit = !!controlToEdit?.Valor;

	const initial = useMemo<CrearControlData>(() => {
		if (controlToEdit) {
			const hora = String(controlToEdit.HoraControl || "").slice(0, 5);
			return {
				numeroVisita: controlToEdit.NumeroVisita || defaultNumeroVisita || 0,
				fechaControl: controlToEdit.FechaControl || getLocalDate(new Date()),
				horaControl: hora || getLocalTime(new Date()),
				operadorCarga: Number(controlToEdit.OperadorCarga) || operadorId,
				idSector: controlToEdit.IdSector || idSector,
				pulso: controlToEdit.Pulso || undefined,
				presionMax: controlToEdit.Maximo || undefined,
				presionMin: controlToEdit.Minimo || undefined,
				presionMedia: controlToEdit.PAMedia || undefined,
				frecuenciaRespiratoria: controlToEdit.FrecuenciaRespiratoria || undefined,
				temperaturaAxilar: controlToEdit.Axilar || undefined,
				temperaturaRectal: controlToEdit.Rectal || undefined,
				glucemia: controlToEdit.Hgt && String(controlToEdit.Hgt) !== "0" ? Number(controlToEdit.Hgt) : undefined,
				saturacion: controlToEdit.Saturometria || undefined,
				peso: controlToEdit.Peso || undefined,
				talla: controlToEdit.Talla || undefined,
				observaciones: controlToEdit.Observaciones || "",
			};
		}
		return {
	...
	}, [controlToEdit, defaultNumeroVisita, operadorId, idSector]);
	...
	return (
		<form id="nuevo-control-form" onSubmit={handleSubmit} className={styles.form}>
			...
			<div className={styles.row}>
				<div className={styles.field}>
					<label className={styles.label}>
						Pulso <span className={styles.unit}>(lat/min)</span>
					</label>
					<input
						className={styles.input}
						type="number"
						...
						max={300}
						value={form.pulso ?? ""}
						onChange={setNum("pulso")}
						placeholder="—"
					/>
				</div>
				<div className={styles.field}>
					<label className={styles.label}>
						TA Máx <span className={styles.unit}>(mmHg)</span>
					</label>
					<input
						className={styles.input}
						type="number"
				...
				<div className={styles.field}>
					<label className={styles.label}>
						TA Mín <span className={styles.unit}>(mmHg)</span>
					</label>
					<input
						className={styles.input}
						type="number"
			...
			<div className={styles.row}>
				<div className={styles.field}>
					<label className={styles.label}>
						Temp. Axilar <span className={styles.unit}>(°C)</span>
					</label>
					<input
						className={styles.input}
						type="number"
						...
						onChange={setNum("temperaturaAxilar")}
						placeholder="—"
					/>
				</div>
				<div className={styles.field}>
					<label className={styles.label}>
						Saturación <span className={styles.unit}>(%)</span>
					</label>
					<input
						className={styles.input}
						type="number"
						...
						onChange={setNum("saturacion")}
						placeholder="—"
					/>
				</div>
				<div className={styles.field}>
					<label className={styles.label}>
						Frec. Respiratoria <span className={styles.unit}>(rpm)</span>
					</label>
					<input
						className={styles.input}
						type="number"
						...
						onChange={setNum("frecuenciaRespiratoria")}
						placeholder="—"
					/>
				</div>
			</div>
			...
			<div className={styles.row}>
				<div className={styles.field}>
					<label className={styles.label}>
						Glucemia <span className={styles.unit}>(mg/dL)</span>
					</label>
					<input
						className={styles.input}
						type="number"
						min={0}
						value={form.glucemia ?? ""}
						onChange={setNum("glucemia")}
						placeholder="—"
					/>
				</div>
				<div className={styles.field}>
					<label className={styles.label}>
						Peso <span className={styles.unit}>(kg)</span>
					</label>
					<input
						className={styles.input}
						type="number"
				...
				<div className={styles.field}>
					<label className={styles.label}>
						Talla <span className={styles.unit}>(cm)</span>
					</label>
					<input
						className={styles.input}
						type="number"
				...
				<div className={styles.field}>
					<label className={styles.label}>
						IMC <span className={styles.unit}>(kg/m²)</span>
					</label>
			...
			<div className={styles.footer}>
				...
				<button type="submit" className={styles.btnPrimary} disabled={saving}>
					{saving ? "Guardando…" : isEdit ? "Guardar cambios" : "Guardar control"}
				</button>
			</div>
		</form>
	);
